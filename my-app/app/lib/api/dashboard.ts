...
export async function fetchDashboardData(
  ...
  // ─── Retrieve Personalized Goals from Backend (GET /profile/:userId) ───
  if (userId && token) {
    try {
      const profile = await getUserProfile(userId, token);
      if (profile) {
        if (profile.targetCalories) {
          savedGoals.calories = profile.targetCalories;
        }
        if (profile.targetProtein) {
          savedGoals.protein = profile.targetProtein;
        }
        if (profile.targetCarbs) {
          savedGoals.carbs = profile.targetCarbs;
        }
        if (profile.targetFat) {
          savedGoals.fat = profile.targetFat;
        }
      }
    } catch (err) {
      console.warn('Could not load user profile goals from backend, using defaults:', err);
    }
  }
  ...
  try {
    ...
    console.warn('Backend food-logs fetch failed, using local dashboard cache:', err);
  }


  // Fallback for empty day or offline: Pass [] to transformBackendLogs
  return transformBackendLogs(dateStr, isToday, [], savedGoals);
}
